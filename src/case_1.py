...
model = BM_two_pool(B0=3, B1=2, Tsat=15, Td=6.5e-3, offset_b=1.9)
paras = np.array([3, 1.05, 2, 0.1, 50, 5e-4])  # (T1a, T1b, T2a, T2b, R, M0b)
Z_my = model.forward(offset, *paras)
df = pd.DataFrame(Z_my)
df.to_excel(os.path.join(base_dir, "results", "case_1.xlsx"), header=False, index=False)
